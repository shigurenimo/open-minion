import { z } from "zod";
import { DISCORD_PET_ID_PREFIX } from "../../../../../lib/engine/discord/discord-pet-source.js";
...
export default factory.createHandlers(helpGuard(help), bodyValidator(schema), async (c) => {
    ...
    if (config.get("discord.enabled") === "false") {
        lines.push("");
        lines.push("Discord 連携は無効化されています (`minion config set discord.enabled true` で戻せます)。");
        return c.text(lines.join("\n"));
    }
    if (!token || !guildId) {
        lines.push("");
        lines.push("Discord 連携は無効です。`minion discord status -h` でセットアップ手順を表示します。");
        return c.text(lines.join("\n"));
    }
    lines.push("");
...
async function describeGateway(minion) {
