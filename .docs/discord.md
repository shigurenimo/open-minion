...
- 見えるのはその私設サーバーに参加しているメンバーだけ。フレンド全員ではない
- メンバー一覧は接続時の GUILD_CREATE 基準(identify の `large_threshold: 250` まで)。
  250 人を超えるギルドでは接続時にオフラインだったメンバーが一覧から欠け、
...
- `discord.enabled` — `false` で連携を止める(設定は残る)。それ以外の値は有効

Claude Code セッション側のペットも同様に `claude.enabled` = `false` で止められる。

⚠️ トークンは `~/.config/minion/config.json` に平文で保存される。このファイルを
